...
const NPM_DIR = join(process.cwd(), '..', 'products', 'npm');
...
test.describe('npm SDK: Exports', () => {
  ...
  test('[exports] Client instantiates with API key', async () => {
    const mod = require(join(NPM_DIR, 'dist'));
    const client = new mod.FortressClient('fk_test123');
...
test.describe('npm SDK: Execute', () => {
  test('[execute] optimize() returns valid result', async () => {
    const mod = require(join(NPM_DIR, 'dist'));
    const client = new mod.FortressClient(apiKey, { baseUrl: API });

    const result = await client.optimize(
      'Please provide a very detailed and comprehensive analysis of the data',
      'balanced',
      'openai',
    );
  ...
  test('[execute] healthCheck() returns result', async () => {
    const mod = require(join(NPM_DIR, 'dist'));
    const client = new mod.FortressClient(apiKey, { baseUrl: API });
    ...
    try {
      ...
      // healthCheck may not exist on all client versions — just verify the method exists
