...
export class DashboardPanel {
  ...
  private constructor(
    ...
    serviceClient: OptimizationServiceClient
  ) {
  ...
  private cardHtml(title: string, before: number, after: number, costSaved: number, count: number): string {
    const percent = before > 0 ? ((before - after) / before) * 100 : 0;
    const pretty = (n: number) => n.toLocaleString(undefined, { maximumFractionDigits: 1 });
    const tokens = (n: number) => n.toLocaleString(undefined, { maximumFractionDigits: 0 });
    return `
