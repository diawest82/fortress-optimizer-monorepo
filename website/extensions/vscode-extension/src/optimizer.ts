export type OptimizerOptions = {
  ...
  provider?: 'openai' | 'anthropic' | 'copilot' | 'claude-desktop' | 'custom';
  detectCode?: boolean;
...
export class Optimizer {
  private SEMANTIC_SIMILARITY_THRESHOLD = 0.85;
  ...
  private getSemanticThreshold(level: string, customThreshold?: number): number {
    if (customThreshold !== undefined) return customThreshold;
  ...
  private removeCodeBoilerplate(text: string, provider: string): string {
    let result = text;
  ...
  optimize(rawPrompt: string, opts: OptimizerOptions): OptimizationResult {
    ...
    const seen: { key: string; original: string }[] = [];
    const kept: string[] = [];
  ...
  noop(rawPrompt: string, opts: OptimizerOptions): OptimizationResult {
